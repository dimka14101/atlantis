...
const char* APssid     = "Atlantis-Kitchen-Access-Point";
...
String mqttlogin;
String mqttpassword;
String mqttpubname = "atlantis/kitchen";
String mqttsubname = "atlantis/kitchennotify";
...
//MQ135 sensor config
#define MQ135AnalogPin 39
#define MQ135DigitalPin 18
...
void setup()
{
  ...
  client.setBufferSize(460); //increase if data not sent
...
void loop() {
  ...
  //Update MQ2 state
  Serial.println("---MQ2 STATE---");
  MQ2SensorValue = analogRead(MQ2pin);
  Serial.print("   |   Current Value = ");  Serial.println(MQ2SensorValue);

  //Update MQ5 state
  Serial.println("---MQ5 STATE---");
  MQ5SensorValue = analogRead(MQ5pin);
  ...
  Serial.println("----TIME----");
  Serial.print("   |   Time="); Serial.println(timeStringBuff);
  ...
  if (now - lastMsg > 5000) {
    ...
    object["MQ2V"] = MQ2SensorValue;
    object["MQ5V"] = MQ5SensorValue;
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == "esp32/kitchenComm") {
      ...
      if (client.available()) {                   // if there's bytes to read from the client,
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            client.println("<title>Atlantis Kitchen Config Web Server</title>");
            ...
            client.println("<h1>Atlantis Kitchen Config Web Server</h1>");
