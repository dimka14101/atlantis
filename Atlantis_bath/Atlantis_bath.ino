...
const char* APssid     = "Atlantis-Bath-Access-Point";
...
String mqttlogin;
String mqttpassword;
String mqttpubname = "atlantis/bath";
String mqttsubname = "atlantis/bathnotify";
...
//Relay inputs config
const int relayIN1Pin = 23;

//Water level config
#define waterLvlPin 39
int waterLvl;
...
void loop() {
  ...
  Serial.println("---WATER LEVEL STATE---");
  Serial.print("   |   Current level = ");
  waterLvl = analogRead(waterLvlPin);
  Serial.println(waterLvl);

  //Update relay state
  Serial.println("---RELAY IN1 STATE---");
  Serial.print(waterLvl < 3000);
  if (waterLvl < 3000) {
    Serial.print("   |   Water DETECTED. Relay ON ");
    digitalWrite(relayIN1Pin, LOW);
  } else {
    Serial.print("   |   Water OK. Relay off ");
    digitalWrite(relayIN1Pin, HIGH);
  }
  Serial.print("   |   Current level = ");
  Serial.println(digitalRead(relayIN1Pin));
  ...
  if (now - lastMsg > 5000) {
    ...
    object["waterLvl"] = analogRead(waterLvlPin);
    object["relayIN1"] = digitalRead(relayIN1Pin);
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == "esp32/bathComm") {
      ...
      if (client.available()) {                   // if there's bytes to read from the client,
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            client.println("<title>Atlantis Bath Config Web Server</title>");
            ...
            client.println("<h1>Atlantis Bath Config Web Server</h1>");
